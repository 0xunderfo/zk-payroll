  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border-2 border-dashed border-gray-600 rounded-xl p-6 text-center hover:border-gray-500 transition-colors">
        ...
        <label htmlFor="csv-upload" className="cursor-pointer">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-gray-300">Upload CSV file</p>
            <p className="text-gray-500 text-sm">Format: address,amount (max {MAX_RECIPIENTS} recipients)</p>
          </div>
        </label>
      </div>
        ...
        {entries.map((entry, index) => (
          <div key={index} className="flex gap-3 items-center">
            ...
            <input
              ...
              className="w-32 bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500"
            />
            <span className="text-gray-500 text-sm">USDT</span>
            {entries.length > 1 && (
              <button onClick={() => removeEntry(index)} className="p-2 text-gray-500 hover:text-red-400">
      ...
      {entries.length < MAX_RECIPIENTS && (
        <button
          onClick={addEntry}
          className="w-full py-2 border border-gray-600 rounded-lg text-gray-400 hover:border-gray-500 hover:text-gray-300 transition-colors"
        >
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-700">
        <div>
          ...
          <p className="text-2xl font-bold">{calculateTotal().toFixed(2)} USDT</p>
        </div>
        <button
          onClick={handleSubmit}
          className="py-3 px-6 bg-blue-600 hover:bg-blue-500 rounded-xl font-semibold transition-colors"
        >
          Prepare Payroll
        </button>
      </div>
